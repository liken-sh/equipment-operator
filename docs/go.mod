// The docs domain is its own Go module, nested inside the operator's.
// Hugo pins hundreds of modules, and none of them belong in the
// operator's go.sum, so the site's dependency graph lives here and
// the program's build never sees it.
//
// Hugo itself enters as a tool directive. `go tool hugo` builds the
// pinned version from source and runs it, so the site needs no Hugo
// install and no fetched binary, only the Go toolchain the operator
// already requires.
module github.com/liken-sh/equipment-operator/docs
